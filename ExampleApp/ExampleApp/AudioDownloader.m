...
{
    if (streamLocation == self.currentStreamLocation)
        return;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self setCurrentStreamLocation:streamLocation];
        NSLog(@"current location %@ new location %@", self.currentStreamLocation, streamLocation);
        if ([self.currentStreamLocation rangeOfString:@"m3u8"].location != NSNotFound) {
            [self createM3u8Session:self.currentStreamLocation];
        }
        else if ([self.currentStreamLocation rangeOfString:@"m3u"].location != NSNotFound) {
            [self getM3uURLAndPlay:self.currentStreamLocation];
        }
        else {
            [self.audioPlayer playURL:[NSURL URLWithString:self.currentStreamLocation]];
        }
    });
}
                                               ...
                                               for (NSString *aLine in lines) {
                                                ...
                                                if ([newFiles count] > 0) {
                                                    for (NSString* aFile in newFiles) {
                                                        NSURL *fileURL = [NSURL URLWithString:aFile];
                                                        STKDataSource* dataSource = [STKAudioPlayer dataSourceFromURL:fileURL];
                                                        SampleQueueId *newQueueItem = [[SampleQueueId alloc] initWithUrl:fileURL andCount:0];
                                                        [self.audioPlayer queueDataSource:dataSource withQueueItemId:newQueueItem];
